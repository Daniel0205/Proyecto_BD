...
class ListaViaje extends Component {
  ...
  render() {
    ...
    let viajesArray = viajes.map(x => {
      return <ConsultaViaje key={x.id} user={this.props.user} datos={x} />;
    });

    if (viajes.length === 0)
      viajesArray = <h2>No se encontraron viajes realizados</h2>;
    /**/
    return (
      <div className="mainv">
        <h1 id="vi">Mis viajes</h1>
        <div id="trips">{viajesArray}</div>
